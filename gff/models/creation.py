...
def get_norms(C):
    ...
    era5_norm = gff.normalisation.get_era5_norm()[C["era5_keys"]]
    ...
    era5l_norm = gff.normalisation.get_era5l_norm()[C["era5_land_keys"]]
    ...
    if "hydroatlas" in C["data_sources"]:
        hydroatlas_norm = gff.normalisation.get_hydroatlas_norm()[C["hydroatlas_bands"]]
        ...
        norms["hydroatlas"] = (hydroatlas_mean, hydroatlas_std)

    if "dem_local" in C["data_sources"] or "dem_context" in C["data_sources"]:
        dem_norm = gff.normalisation.get_dem_norm(fold=C["fold"])
    ...
    if "s1" in C["data_sources"]:
        s1_norm = gff.normalisation.get_s1_norm(fold=C["fold"])
